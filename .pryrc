  ...
  def process
    ...
    if last_exception.class == NameError
      ...
      if Rails != nil?
        raise
        if klass =~ /.*Controller$/
          code = "class #{klass} < ApplicationController\nend\n"
          file = File.join(Rails.root, 'app', 'controllers', file)
        else
          code = "class #{klass} < ActiveRecord::Base\nend\n"
          file = File.join(Rails.root, 'app', 'models', file)
        end
      end
